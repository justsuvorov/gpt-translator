...
from src.ml.provider import OpenAIModelProvider


class FineTuner:
    """Управляет fine-tuning моделей OpenAI."""

    def __init__(self, provider: OpenAIModelProvider):
        self.provider = provider
        self.client = provider.get_client()
        self.base_model = provider.get_base_model()
        self.config = provider.get_config()
        self.suffix = self.config["fine_tuning"].get("suffix", "translation")
    ...
    def list_jobs(self) -> list[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Failed to list jobs: {e}", exc_info=True)
            raise
